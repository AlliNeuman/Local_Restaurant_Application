...
["pappy's smokehouse", "midtown", "3106 olive", "barbecue", "Try the Man vs. Food challenge, or the ribs! Get there early so they're not out of food. Better yet, call ahead and pick up so you can skip the line.", User.find_by_username("dave")],
["venice cafe", "lafayette square", "1903 pestalozzi", "caribbean", "Get a bucket with your friends (choose your alcohol of preference and let them dump the rest in)! Cash only bar with a cover charge on Friday and Saturday for live music.", User.find_by_username("colin")],
["farmhaus",  "south city",  "3257 Ivanhoe", "american",  "If you're looking for small delicious bites, this is the one! An instagrammer heaven. Save up a few special dates for this one, it's a bit pricey.",  User.find_by_username("alli")],
["hodak's restaurant",  "south county",  "2100 gravois",  "southern", "The cheapest, delicious half fried chicken you'll find in St Louis!",  User.find_by_username("stacy")],
["crown candy kitchen",  "north city",  "1401 st louis",  "american",  "Don't try the Man vs. Food challenge unless you want to throw up. Get a malt shake and 'heart stopping blt'!",  User.find_by_username("colin")],
["brasserie by niche",  "central west end",  "4580 laclede",  "french",  "Get their burger! It's like a fancy version of an In and Out burger. Book a week ahead on open table",  User.find_by_username("alli")],
["scape",  "central west end",  "48 maryland plaza",  "american",  "Go for their brunch! Get their chilaquiles. Great bloody mary bar! Reserve on open table ahead of time.",  User.find_by_username("Dave")],
["blueberry hill",  "u city",  "6504 delmar",  "american",  "Bomb salad, great burger and hotdog. Make a night of it by playing their arcade games or darts. Must be 21 after a certain hour of night so be careful! Great for people watching.",  User.find_by_username("colin")],
["pho long",  "u city",  "8627 olive",  "vietnamese",  "Personal favorite - best pho in St Louis!",  User.find_by_username("alli")],
["bar les freres",  "clayton",  "7637 wydown",  "french", "Nice spot to celebrate a birthday or for a romantic date. Go for drinks and appetizers. Schedule reservation well in advance.",  User.find_by_username("stacy")],
["city coffee house & creperie",  "clayton",  "36 n brentwood",  "cafe",  "The St Louis crepe is amazing. Skip the smoothie and save all your room for a the crepe!", User.find_by_username("alli") ] ,
["half & half",  "clayton",  "8135 maryland", "american", "Delicious and popular brunch spot. Make sure to use the No Wait app ahead of time. Wait times can be 60 to 200 minutes! Make sure to arrive a few minutes early to get a mocha with some coffee art"],  User.find_by_username("alli")],
["half & half",  "clayton",  "8135 maryland", "american", "Delicious and popular brunch spot. Make sure to use the No Wait app ahead of time. Wait times can be 60 to 200 minutes! Make sure to arrive a few minutes early to get a mocha with some coffee art", User.find_by_username("alli")],
["pastaria", "clayton", "7734 Forsyth", "italian", "I would consider this a modern italian posh place. The pappardelle is amazing! Adult beverages are pretty strong. Use the No Wait app ahead of time. Wait times can be 30-220 minutes EEK!",  User.find_by_username("dave")],
["bricktop's",  "frontenance",  "10342 Clayton",  "american", "Ribs are delicious. Take your own bottle of wine! There's no corkage fee.",  User.find_by_username("dave")],
["bailey's chocolate bar",  "lafayette square",  "1915 Park",  "martini bar",  "Get one of their amazing frozen martinis. Great spot for a romantic date!", User.find_by_username("alli")],
["seamus mcdaniels", "dogtown",  "1208 tamm ave.", "bar & grill",  "Get their burger and ask for a side of ranch!", User.find_by_username("alli")],
["favazzas", "the jill", "5201 sw ave.", "italian",  "Order the baked pasta with meatballs!",  User.find_by_username("dave")],
["bar louie",  "central west end",  "14 maryland plaza",  "american",  "Go on Tuesday's for dollar burgers!",  User.find_by_username("colin")],
["carl's drive in", "maplewood", "9033 Manchester", "american", "MUST HAVES: home made rootbeer and a triple or quad (not on the menu) cheeseburger! Onion rings and chili are also bomb. The cheeseburgers have cripsy edges and do not disappoint!", User.find_by_username("dave")],
["mai lee", "brentwood", "8396 musick memorial", "vietnamese", "Get the pho. Order the fresh spring rolls in the rice paper with beef instead.", User.find_by_username("colin")],
["citizen kane's steakhouse", "kirkwood", "133 w. clinton", "steakhouse", "You could literally cut their filet with a butter knife. It's just that juicy and good.", User.find_by_username("stacy")],
["cyrano's", "webster groves", "603 e. lockwood", "cafe", "Skip the food and just come for dessert. Get the Cleopatra and share with your friends. It's like a mile high", User.find_by_username("alli")],
["cyrano's", "webster groves", "603 e. lockwood", "cafe", "Skip the food and just come for dessert. Get the Cleopatra and share with your friends. It's like a mile high", User.find_by_username("alli")],
["hacienda", "rock hill", "9748 Manchester", "mexican", "Arrive early to get patio seating. Food is meh, but outside seating is great. Go for margaritas!", User.find_by_username("alli")],
["olive + oak", "webster groves", "102 w lockwood", "american", "A restaurant with a great cause. Two families lost their children to a rare disease and their restaurant profit goes towards the cause. Plan at minimum 3 months ahead and schedule online through their website.", User.find_by_username("alli")],
["spencer's grill", "kirkwood", "223 S Kirkwood", "diner", "Get their pancakes, you won't regret it!", User.find_by_username("colin")],
["paul manno's cafe", "west county", "75 forum center", "italian", "Book reservation well in advance. They have regulars that go in every weekend so reservations are tough to get last minute.", User.find_by_username("dave")],
["ya ya's euro bistro", "west county", "15601 Olive", "european", "Go for their brunch, it's unlimited food! Book on open table.", User.find_by_username("dave")],
["gamlin whiskey house", "central west end", "236 n euclid", "steakhouse", "Go for brunch, not steak. Get a special and a ghost pepper vodka bloody mary.", User.find_by_username("colin")],
["hi pointe drive in", "hi pointe", "1033 mccausland", "comfort food", "Look at their Facebook or Instagram ahead of time to see their daily specials or recent specials. They can sometimes make recent specials if you request it.", User.find_by_username("alli")],
["momos ouzaria taverna", "u city", "630 n & s", "greek", "Great tapas, but especially delicious melting chocolate dessert in a pastry dough.", User.find_by_username("dave")],
["boundary", "richmond heights", "7036 Clayton", "american", "get the steak tartare and a craft cocktail!", User.find_by_username("dave")],
]

bookmarks_list [
  ["true", Restaurant.find_by_name("carl's drive in").id, User.find_by_username("alli").id],
  ["true", Restaurant.find_by_name("blueberry hill").id, User.find_by_username("alli").id],
  ["false", Restaurant.find_by_name("gamlin whiskey house").id,  User.find_by_username("alli").id],
  ["true", Restaurant.find_by_name("kemolls").id, User.find_by_username("colin").id],
  ["false", Restaurant.find_by_name("citizen kane's steakhouse").id,  User.find_by_username("colin").id],
  ["false", Restaurant.find_by_name("spencer's grill").id,  User.find_by_username("stacy").id],
  ["true", Restaurant.find_by_name("favazzas").id,  User.find_by_username("stacy").id],
  ["true", Restaurant.find_by_name("seamus mcdaniels").id, User.find_by_username("dave").id],
  ["false", Restaurant.find_by_name("mai lee").id, User.find_by_username("dave").id],
]
...
bookmarks_list.each do |bookmark|
  Bookmark.create(:visited => bookmarkt[0], :restaurant_id => bookmark[1], :user_id => bookmark[2])
end 
